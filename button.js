...
const getStreak = async (button) => {
  db.get(button.clicker.user.id + "").then(logs => {
      if (logs != null && typeof logs.info !== 'undefined')
        ...
        embedder(button, `**${button.clicker.user.username}**, you do not have any current logs. Use \`++add {log}\` to start logging.`)
    })
}
...
const getProfile = async (button) => {
  db.get(button.clicker.user.id).then(logs => {
    if(logs!=null && typeof logs.info !== 'undefined')
      ...
      msg.reply(`*${button.clicker.user.username}*, You don't have any logs currently. Start logging with us to check out your profile.`)
  })
}
...
const getList = async (button) => {
  db.get(button.clicker.user.id + "").then(logs => {
    ...
    listAll(button, logs.info, logs.open)
      ...
      embedder(msg,  `You do not have any logs ${button.clicker.user.username}. Start logging with \`++add {log}\` to check out your logs.`)
  })
}
